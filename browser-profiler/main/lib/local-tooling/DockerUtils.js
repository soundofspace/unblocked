...
async function startDockerAndLoadUrl(dockerName, dockerHost, url, automationType, needsLocalHost, chromeVersion) {
    ...
    const command = `docker run --init --rm --name ${dockerName} --security-opt seccomp="./Docker-chrome.json" --ipc=host --shm-size='3gb' --cap-add=SYS_ADMIN ${hostArg} ${dockerArgs} ${dockerName} "${chromeArgs.join(' ')}" "${urlArg}"`;
    console.log(command);
