...
class DomBridger {
    ...
    static removeUnsupportedPropertiesFromPolyfill(polyfill) {
        polyfill.modify = polyfill.modify.filter(x => !isUnsupportedProperty(x.path, x.propertyName, x.property));
        polyfill.remove = polyfill.remove.filter(x => !isUnsupportedProperty(x.path, x.propertyName, null));
        polyfill.add = polyfill.add.filter(x => !isUnsupportedProperty(x.path, x.propertyName, x.property));
    }
}
exports.default = DomBridger;
function isUnsupportedProperty(path, propertyName, property) {
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function isHeadlessIndicator(path, propertyName) {
