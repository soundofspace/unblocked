    ...
    static async shouldIntercept(ctx) {
        ...
        if (ctx.proxyToClientResponse) {
            if (await requestSession.didHandleInterceptResponse(ctx, ctx.clientToProxyRequest, ctx.proxyToClientResponse)) {
                ctx.responseHeaders ?? (ctx.responseHeaders = ctx.proxyToClientResponse.getHeaders());
                ctx.responseTime ?? (ctx.responseTime = Date.now());
                ctx.status ?? (ctx.status = ctx.proxyToClientResponse.statusCode);
                ctx.statusMessage ?? (ctx.statusMessage = ctx.proxyToClientResponse.statusMessage);
