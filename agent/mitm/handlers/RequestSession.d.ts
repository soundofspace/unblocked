...
export default class RequestSession extends TypedEventEmitter<IRequestSessionEvents> implements IBrowserContextHooks {
    ...
    interceptorHandlers: {
        types?: IResourceType[];
        urls?: (string | RegExp)[];
        handlerFn?: (url: URL, type: IResourceType, request: http.IncomingMessage | http2.Http2ServerRequest, response: http.ServerResponse | http2.Http2ServerResponse) => boolean | Promise<boolean>;
        hasParsed?: boolean;
    }[];
    ...
    willSendResponse(context: IMitmRequestContext): Promise<void>;
    haveSentResponse(context: IMitmRequestContext): Promise<void>;
    lookupDns(host: string): Promise<string>;
    getProxyCredentials(): string;
    close(): void;
    shouldInterceptRequest(url: string, resourceType?: IResourceType): boolean;
