...
class TestServer {
    constructor(sslOptions) {
        this.dirPath = Path.resolve(__dirname, '../assets');
        this.sockets = new Set();
        this.routes = new Map();
        this.auths = new Map();
        this.csp = new Map();
        this.gzipRoutes = new Set();
        this.requestSubscribers = new Map();
        this.protocol = 'http:';
        if (sslOptions) {
    ...
    serveFile(request, response, filePath) {
        ...
        if (this.gzipRoutes.has(pathName)) {
            stream.pipe((0, zlib_1.createGzip)()).pipe(response);
        }
        else {
