...
class Frame extends eventUtils_1.TypedEventEmitter {
    ...
    constructor(framesManager, internalFrame, activeContextIds, devtoolsSession, logger, checkIfAttached, parentFrame) {
        super();
    ...
    async updateDevtoolsSession(devtoolsSession, activeContextIds) {
    ...
    async runPendingNewDocumentScripts() {
        ...
        if (newDocumentScripts.length) {
            ...
            if (this.closedWithError || !this.devtoolsSession.isConnected())
                return;
            await Promise.all(scripts.map(x => {
                if (this.closedWithError || !this.devtoolsSession.isConnected())
                    return;
  ...
  return retVal;
    ...
    async waitForNavigationLoader(loaderId, timeoutMs) {
        ...
        // }
        await __classPrivateFieldGet(this, _Frame_framesManager, "f").getExecutionContextId({
            frameId: this.id,
            type: 'default',
    ...
    /////// CONTEXT ID  //////////////////////////////////////////////////////////////////////////////////////////////////
    hasContextId(executionContextId) {
        return (this.defaultContextId === executionContextId || this.isolatedContextId === executionContextId);
    }
    removeContextId(executionContextId) {
        if (this.defaultContextId === executionContextId) {
            this.defaultContextId = null;
        }
        if (this.isolatedContextId === executionContextId)
    ...
    addContextId(executionContextId, isDefault, origin) {
        if (isDefault) {
            ...
            if (!!this.getActiveContextId(true) && origin !== '')
                return;
            this.isolatedContextId = executionContextId;
        }
    }
    getActiveContextId(isolatedContext) {
        let id;
        if (isolatedContext) {
        ...
        if (id && this.activeContextIds.has(id))
            return id;
    }
    async waitForActiveContextId(isolatedContext = true) {
        if (!this.isAttached)
        ...
        const contextId = await __classPrivateFieldGet(this, _Frame_framesManager, "f").getExecutionContextId({
            frameId: this.id,
    ...
    canEvaluate(isolatedFromWebPageEnvironment) {
    ...
    async waitForDefaultContext() {
        if (this.getActiveContextId(false))
            return;
        this.defaultContextCreated = new Resolvable_1.default();
        await Promise.all([
