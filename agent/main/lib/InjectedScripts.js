...
exports.injectedScript = `(function ulxInjectedScripts(runtimeFunction) {
...
class InjectedScripts {
    static install(framesManager, devtoolsSession, onPaintEvent) {
        return Promise.all([
            framesManager.addPageCallback(pageEventsCallbackName, (payload, frame) => onPaintEvent(frame.frameId, JSON.parse(payload)), framesManager.page.installJsPathIntoIsolatedContext, devtoolsSession),
