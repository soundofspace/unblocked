...
class Page extends eventUtils_1.TypedEventEmitter {
    constructor(devtoolsSession, targetId, browserContext, logger, opener, pageOptions) {
        ...
        this.networkManager = new NetworkManager_1.default(devtoolsSession, this.logger, this.browserContext.proxy);
    ...
    addNewDocumentScript(script, isolatedEnvironment, devtoolsSession) {
    ...
    addPageCallback(name, onCallback, isolateFromWebPageEnvironment, devtoolsSession) {
        return this.framesManager.addPageCallback(name, (payload, frame) => {
            if (onCallback)
                onCallback(payload, frame);
            this.emit('page-callback-triggered', {
                name,
                payload,
                frameId: frame.frameId,
            });
        }, isolateFromWebPageEnvironment, devtoolsSession);
    }
    ...
    evaluate(expression, isolatedFromWebPageEnvironment = false) {
        return this.mainFrame.evaluate(expression, isolatedFromWebPageEnvironment);
    }
    ...
    onRuntimeException(msg) {
        ...
        const frame = this.framesManager.getFrameForExecutionContext(msg.exceptionDetails.executionContextId);
