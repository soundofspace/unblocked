...
const WebsocketSession_1 = require("./WebsocketSession");
...
class BrowserContext extends eventUtils_1.TypedEventEmitter {
    ...
    constructor(browser, isIncognito, options) {
        ...
        this.websocketSession = new WebsocketSession_1.WebsocketSession();
    }
    async open() {
        if (!this.isIncognito)
            return;
    ...
    async close() {
        ...
        try {
            ...
            for (const waitingPage of this.waitForPageAttachedById.values()) {
                await waitingPage.reject(new IPendingWaitEvent_1.CanceledPromiseError('BrowserContext shutting down'), true);
            }
