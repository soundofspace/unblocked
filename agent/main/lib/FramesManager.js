...
class FramesManager extends eventUtils_1.TypedEventEmitter {
    ...
    constructor(page, devtoolsSession) {
        ...
        this.websocketIdToFrameId = new Map();
    ...
    initialize(devtoolsSession) {
        ...
        // ),
        this.events.on(devtoolsSession, 'Runtime.executionContextCreated', this.onExecutionContextCreated.bind(this, devtoolsSession)), this.events.on(devtoolsSession, 'Runtime.executionContextCreated', this.onExecutionContextCreatedNew.bind(this, devtoolsSession)));
    ...
    async onExecutionContextCreatedNew(devtoolsSession, event) {
        await this.isReady;
        const { context } = event;
        const frameId = context.auxData.frameId;
        const type = context.auxData.type;
    ...
    async getExecutionContextId(opts) {
        const refresh = async () => {
            await Promise.all([
        ...
        const resolvable = new Resolvable_1.default(3e3);
        this.frameToContextId.set(key, resolvable);
        if (!opts.refresh)
            void refresh();
