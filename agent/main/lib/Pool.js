...
class Pool extends eventUtils_1.TypedEventEmitter {
    ...
    async createMitmProxy() {
        this.certificateGenerator ?? (this.certificateGenerator = unblocked_agent_mitm_1.MitmProxy.createCertificateGenerator(this.options.certificateStore, this.options.dataDir));
        return await unblocked_agent_mitm_1.MitmProxy.start(this.certificateGenerator);
    }
    async getBrowser(engine, hooks, launchArgs) {
        return await this.browserCreationQueue.run(async () => {
            ...
            if (needsBrowserLevelProxy) {
                launchArgs.proxyPort ?? (launchArgs.proxyPort = this.sharedMitmProxy?.port);
            }
