...
class Resources extends eventUtils_1.TypedEventEmitter {
    ...
    onBrowserRequestFailed(tabId, frameId, resource, loadError) {
        ...
        resourceId ?? (resourceId = this.getBrowserRequestLatestResource(resource.browserRequestId)?.id);
    ...
    determineResourceType(mitmResource) {
        ...
        if (HeadersHandler_1.default.isWorkerDest(mitmResource, 'shared', 'service') ||
    ...
    updatePendingResource(httpResourceLoad, browserRequest, tabId, frameId) {
        browserRequest.tabId ?? (browserRequest.tabId = tabId);
        browserRequest.frameId ?? (browserRequest.frameId = frameId);
        browserRequest.browserLoadedTime ?? (browserRequest.browserLoadedTime = httpResourceLoad.browserLoadedTime);
