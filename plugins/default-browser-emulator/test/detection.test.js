...
const browserVersion = index_2.default.default().fullVersion.split('.').map(Number).shift();
let koaServer;
let pool;
...
test('should pass FpScanner', async () => {
    ...
    for (const key of Object.keys(results)) {
        ...
        if (!isConsistent && result.name === 'WEBDRIVER')
            continue;
        // eslint-disable-next-line no-console
        if (!isConsistent)
            console.log('Not consistent', result);
...
test('should not call evaluate on a stack getter in debug', async () => {
  ...
  <script>
    ...
    const allowStackGetter = await page.evaluate('window.didCallGetter');
...
test('should be able to post message', async () => {
    ...
    expect(result.plugins.message).toBe("Failed to execute 'postMessage' on 'Window': PluginArray object could not be cloned.");
    expect(result.plugin0.message).toBe("Failed to execute 'postMessage' on 'Window': Plugin object could not be cloned.");
    expect(result.mimes.message).toBe("Failed to execute 'postMessage' on 'Window': MimeTypeArray object could not be cloned.");
    expect(result.mime0.message).toBe("Failed to execute 'postMessage' on 'Window': MimeType object could not be cloned.");
});
...
<script>
  ...
  window.betrayalResult = {
    ...
    await page.waitForLoad('DomContentLoaded');
    const result = await page.evaluate(`window.betrayalResult`);
    ...
    if (browserVersion < 112 && browserVersion > 97) {
        ...
        stack = stack.replace(`at iWillBetrayYouWithMyLongName (${koaServer.baseUrl}/betrayal:5:9)`, `at String.toString (<anonymous>)`);
    }
...
test('should properly emulate memory', async () => {
    ...
    const page = await agent.newPage();
    page.on('console', console.log);
    const server = await index_1.Helpers.runHttpsServer((req, res) => {
...
test('should properly maintain stack traces in toString', async () => {
    ...
    const fnStack = await page.evaluate(`window.hasProperStackTrace(AudioBuffer.prototype.copyFromChannel)`);
    const toStringStack = await page.evaluate(`window.hasProperStackTrace(Function.prototype.toString)`);
    const objectStack = await page.evaluate(`window.hasProperStackTrace(() => {})`);
    const proxiedFnStack = await page.evaluate(`window.hasProperStackTrace(new Proxy(AudioBuffer.prototype.copyFromChannel, {}))`);
    const getterStack = await page.evaluate(`window.hasProperStackTrace(Object.getOwnPropertyDescriptor(Navigator.prototype, 'deviceMemory').get)`);
    const proxiedGetterStack = await page.evaluate(`window.hasProperStackTrace(new Proxy(Object.getOwnPropertyDescriptor(Navigator.prototype, 'deviceMemory').get, {}))`);
...
document.querySelector = (function (orig) {
  return function() {
    ...
    expect(query).toBe('Error: QuerySelector Override Detection\n    at HTMLDocument.querySelector (<anonymous>:4:17)\n    at <anonymous>:1:10');
});
...
describe('Proxy detections', () => {
        ...
        getChainCycleLie(apiFunction, chromeVersion, method = 'setPrototypeOf') {
            try {
                ...
                if (error.message !== `Cyclic __proto__ value`)
                    return 'Not cyclic __proto__';
                const targetStackLine = ((error.stack || '').split('\n') || [])[1];
                if (method === '__proto__') {
                    const targetStack = chromeVersion >= 102
    ...
    test('should not reveal instanceof proxy behavior on getter', async () => {
        ...
        await page.goto(`${koaServer.baseUrl}`);
        page.on('console', console.log);
        const result = await page.evaluate(`(function ${ProxyDetections.checkInstanceof.toString()})(Object.getOwnPropertyDescriptor(Navigator.prototype,'userAgent').get, ${browserVersion});`);
    ...
    test('should not reveal instanceof proxy behavior on fn', async () => {
        ...
        await page.goto(`${koaServer.baseUrl}`);
        page.on('console', console.log);
        const result = await page.evaluate(`(function ${ProxyDetections.checkInstanceof.toString()})(Permissions.prototype.query, ${browserVersion});`);
        expect(result).toBe('ok');
    });
    test('should not reveal recursion errors for getPrototypeOf', async () => {
        ...
        const page = await agent.newPage();
        await page.goto(`${koaServer.baseUrl}`);
        const result = await page.evaluate(`(function ${ProxyDetections.getChainCycleLie.toString()})(Permissions.prototype.query, ${browserVersion})`);
        expect(result).toBe('ok');
    });
    test('should not reveal recursion errors for getPrototypeOf of a getter', async () => {
        ...
        const page = await agent.newPage();
        await page.goto(`${koaServer.baseUrl}`);
        const result = await page.evaluate(`(function ${ProxyDetections.getChainCycleLie.toString()})(Object.getOwnPropertyDescriptor(Navigator.prototype,'userAgent').get, ${browserVersion})`);
        expect(result).toBe('ok');
    });
    test('should not reveal recursion errors for __proto__', async () => {
        ...
        const page = await agent.newPage();
        await page.goto(`${koaServer.baseUrl}`);
        const protoResult = await page.evaluate(`(function ${ProxyDetections.getChainCycleLie.toString()})(Permissions.prototype.query, ${browserVersion}, '__proto__')`);
    ...
    test('should not fail at setting proto of Reflect.setPrototypeOf', async () => {
        ...
        const result = await page.evaluate(`(${getReflectSetProtoLie.toString()})(Permissions.prototype.query)`);
        expect(result).toBe('ok');
    });
...
it('should emulate in a shared worker', async () => {
    const hasAllResults = new Resolvable_1.default();
    const jsonResults = [];
    let postResolvable = new Resolvable_1.default();
    const httpsServer = await index_1.Helpers.runHttpsServer(async (req, res) => {
...
<html lang="en">
      ...
      (async () => {
       ...
       await fetch('/worker-result', {
          ...
          body: JSON.stringify(results),
            ...
            const result = await index_1.Helpers.readableToBuffer(req);
    ...
    for (let i = 0; i < 10; i += 1) {
        postResolvable = new Resolvable_1.default();
