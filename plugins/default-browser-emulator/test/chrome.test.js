...
beforeAll(async () => {
    const selectBrowserMeta = index_1.default.selectBrowserMeta('~ mac = 10.14');
    const { browserVersion, operatingSystemVersion } = selectBrowserMeta.userAgentOption;
    const windowChromePath = Path.resolve(paths_1.emulatorDataDir, `as-chrome-${browserVersion.major}-0/as-mac-os-${operatingSystemVersion.major}-${operatingSystemVersion.minor}/window-chrome.json`);
...
test('it should mimic a chrome object', async () => {
    ...
    const structure = JSON.parse((await page.mainFrame.evaluate(`new (${DomExtractor.toString()})('window').run(window, 'window', ['chrome'])`, false))).window;
    if (debug)
    ...
    const structureJson = JSON.stringify(structure.chrome, (key, value) => {
        if (key === '_$value' || key === '_$invocation')
            return undefined;
        return value;
    });
...
test('it should update loadtimes and csi values', async () => {
    ...
    const loadTimes = JSON.parse((await page.mainFrame.evaluate(`JSON.stringify(chrome.loadTimes())`, false)));
    if (debug)
    ...
    const csi = JSON.parse((await page.mainFrame.evaluate(`JSON.stringify(chrome.csi())`, false)));
    if (debug)
