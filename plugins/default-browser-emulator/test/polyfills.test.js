...
test('it should be able to add polyfills', async () => {
    ...
    await page.addNewDocumentScript((0, DomOverridesBuilder_1.getOverrideScript)('polyfill.add', {
        itemsToAdd: [
            {
                path: 'window',
    ...
    const json = await page.mainFrame.evaluate(`new (${DomExtractor.toString()})('window').run(window, 'window', ['windowKeys','chromey','ObjectTest'])`, false);
...
test('it should be able to remove properties', async () => {
    const page = await createPage();
    await page.addNewDocumentScript((0, DomOverridesBuilder_1.getOverrideScript)('polyfill.remove', {
        itemsToRemove: [
...
test('it should be able to change properties', async () => {
    const page = await createPage();
    await page.addNewDocumentScript((0, DomOverridesBuilder_1.getOverrideScript)('polyfill.modify', {
        itemsToModify: [
            {
    ...
    const protocolToString = await page.mainFrame.evaluate(`window.Navigator.prototype.registerProtocolHandler.toString()`, false);
    const protocolName = await page.mainFrame.evaluate(`window.Navigator.prototype.registerProtocolHandler.name`, false);
...
test('it should be able to change property order', async () => {
    const page = await createPage();
    const startNavigatorKeys = (await page.mainFrame.evaluate(`Object.keys(window.Navigator.prototype)`, false));
    await page.addNewDocumentScript((0, DomOverridesBuilder_1.getOverrideScript)('polyfill.reorder', {
        itemsToReorder: [
            {
    ...
    const keyOrder = (await page.mainFrame.evaluate(`Object.keys(window.Navigator.prototype)`, false));
...
test('it should be able to change window property order', async () => {
    ...
    const windowKeysAfter = (await page.mainFrame.evaluate(`Object.keys(window)`, false));
