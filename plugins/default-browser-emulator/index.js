...
exports.defaultBrowserEngine = void 0;
...
let DefaultBrowserEmulator = DefaultBrowserEmulator_1 = class DefaultBrowserEmulator {
    constructor(emulationProfile) {
        ...
        emulationProfile.deviceProfile ?? (emulationProfile.deviceProfile = {});
    ...
    get domOverridesBuilder() {
        __classPrivateFieldSet(this, _DefaultBrowserEmulator_domOverridesBuilder, __classPrivateFieldGet(this, _DefaultBrowserEmulator_domOverridesBuilder, "f") ?? (0, loadDomOverrides_1.default)(this.emulationProfile, this.data, this.userAgentData), "f");
        return __classPrivateFieldGet(this, _DefaultBrowserEmulator_domOverridesBuilder, "f");
    }
    configure(emulationProfile) {
        emulationProfile.windowNavigatorPlatform ?? (emulationProfile.windowNavigatorPlatform = this.data.windowNavigator.navigator.platform._$value);
        emulationProfile.locale ?? (emulationProfile.locale = this.data.browserConfig.defaultLocale);
        emulationProfile.viewport ?? (emulationProfile.viewport = Viewports_1.default.getDefault(this.data.windowBaseFraming, this.data.windowFraming));
        emulationProfile.timezoneId ?? (emulationProfile.timezoneId = Intl.DateTimeFormat().resolvedOptions().timeZone);
    ...
    async onHttpAgentInitialized(agent) {
        ...
        if (upstreamProxyIpMask && profile.upstreamProxyUrl) {
            upstreamProxyIpMask.publicIp ?? (upstreamProxyIpMask.publicIp = await (0, lookupPublicIp_1.default)(upstreamProxyIpMask.ipLookupService));
            upstreamProxyIpMask.proxyIp ?? (upstreamProxyIpMask.proxyIp = await (0, lookupPublicIp_1.default)(upstreamProxyIpMask.ipLookupService, agent, profile.upstreamProxyUrl));
    ...
    static selectBrowserMeta(userAgentSelector) {
        ...
        const browserEngine = new BrowserEngine_1.default(browserEngineOption);
