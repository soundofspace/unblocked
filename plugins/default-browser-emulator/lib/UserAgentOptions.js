...
class UserAgentOptions {
    ...
    findClosestInstalled(userAgent) {
        ...
        const withOs = filteredOptions.filter(x => x.operatingSystemName === userAgent.operatingSystemName);
        if (withOs.length)
    ...
    static convertToUserAgentOption(agent) {
        ...
        const uaClientHintsPlatformVersion = (0, utils_1.pickRandom)(agent.uaClientHintsPlatformVersions);
        const userAgent = {
    ...
    static canTrustOsVersionForAgentString(agentOption) {
        ...
        if (agentOption.operatingSystemName === 'mac-os' &&
        ...
        if (agentOption.operatingSystemName === 'windows' &&
            agentOption.operatingSystemVersion.major === '10') {
            return false;
        }
        return true;
...
function cleanName(name) {
    cleanCache[name] ?? (cleanCache[name] = name.toLowerCase().replace(/[^a-z]+/, '-'));
