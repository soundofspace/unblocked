...
exports.configureBrowserLaunchArgs = void 0;
const Path = require("path");
const os = require("os");
const nanoid_1 = require("nanoid");
const Viewports_1 = require("../Viewports");
const instanceId = (0, nanoid_1.nanoid)(5);
function configureBrowserLaunchArgs(engine, options) {
    engine.launchArguments.push('--disable-background-networking', // Disable various background network services, including extension updating,safe browsing service, upgrade detector, translate, UMA
    '--enable-features=NetworkService,NetworkServiceInProcess', '--disable-background-timer-throttling', // Disable timers being throttled in background pages/tabs
    '--disable-backgrounding-occluded-windows', '--disable-breakpad', // Disable crashdump collection (reporting is already disabled in Chromium)
     ...
     */
    '--disable-features=PaintHolding,LazyFrameLoading,DestroyProfileOnBrowserClose,AvoidUnnecessaryBeforeUnloadCheckSync,OutOfBlinkCors,GlobalMediaControls,MediaRouter,DialMediaRouteProvider', '--disable-blink-features=AutomationControlled', '--disable-hang-monitor', '--disable-ipc-flooding-protection', // Some javascript functions can be used to flood the browser process with IPC. By default, protection is on to limit the number of IPC sent to 10 per second per frame.
    ...
    '--use-fake-device-for-media-stream', '--no-default-browser-check', //  Disable the default browser check, do not prompt to set it as such
    ...
    '--allow-running-insecure-content', `--window-size=${Viewports_1.defaultScreen.width},${Viewports_1.defaultScreen.height}`, 
    ...
    if (options.showChrome) {
        if (options.showDevtools)
            engine.launchArguments.push('--auto-open-devtools-for-tabs');
    }
    else {
        engine.launchArguments.push('--hide-scrollbars', '--mute-audio', '--blink-settings=primaryHoverType=2,availableHoverTypes=2,primaryPointerType=4,availablePointerTypes=4');
    }
    if (options.disableGpu === true) {
        ...
        if (idx >= 0)
            engine.launchArguments.splice(idx, 1);
    }
}
exports.configureBrowserLaunchArgs = configureBrowserLaunchArgs;
//# sourceMappingURL=configureBrowserLaunchArgs.js.map