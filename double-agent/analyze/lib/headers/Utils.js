"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.isOfficialDefaultValueKey = exports.isOfficialHeader = void 0;
function isOfficialHeader(key) {
    const keyLower = key.toLowerCase();
    for (const prefix of officialHeaderPrefixes) {
        if (keyLower.startsWith(prefix))
            return true;
    }
    return officialHeaderKeys.has(keyLower);
}
exports.isOfficialHeader = isOfficialHeader;
function isOfficialDefaultValueKey(key) {
    return officialDefaultValueKeys.has(key.toLowerCase());
}
exports.isOfficialDefaultValueKey = isOfficialDefaultValueKey;
/////// /////////
const officialHeaderPrefixes = [
    'sec-',
    'proxy-',
    'access-control-',
];
const officialHeaderKeys = new Set([
    ':method',
    ':authority',
    ':scheme',
    ':path',
    'accept',
    'accept-charset',
    'accept-encoding',
    'accept-language',
    'accept-patch',
    'accept-ranges',
    'age',
    'allow',
    'alt-svc',
    'authorization',
    'cache-control',
    'connection',
    'content-disposition',
    'content-encoding',
    'content-language',
    'content-length',
    'content-location',
    'content-range',
    'content-type',
    'cookie',
    'date',
    'expect',
    'expires',
    'forwarded',
    'from',
    'host',
    'if-match',
    'if-modified-since',
    'if-none-match',
    'if-range',
    'if-unmodified-since',
    'last-modified',
    'location',
    'origin',
    'pragma',
    'public-key-pins',
    'range',
    'referer',
    'retry-after',
    'set-cookie',
    'strict-transport-security',
    'tk',
    'trailer',
    'transfer-encoding',
    'upgrade',
    'upgrade-insecure-requests',
    'user-agent',
    'vary',
    'via',
    'warning',
    'www-authenticate',
]);
const officialDefaultValueKeys = new Set([
    'connection',
    'sec-fetch-site',
    'sec-fetch-mode',
    'sec-fetch-user',
    'sec-fetch-dest',
    'sec-ch-ua-mobile',
    'sec-ch-ua',
    'sec-ch-ua-platform',
    'sec-websocket-version',
    'sec-websocket-extensions',
    'accept',
    'accept-encoding',
    'accept-language',
    'upgrade',
    'upgrade-insecure-requests',
]);
//# sourceMappingURL=Utils.js.map