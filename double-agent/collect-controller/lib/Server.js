...
class Server {
    ...
    async sendFavicon(_, res) {
        this.favicon ?? (this.favicon = await fs_1.promises.readFile(`${__dirname}/../public/favicon.ico`));
...
async function pipeDirToStream(dirPath, stream) {
    ...
    if (await existsAsync(dirPath)) {
        const fileNames = await fs_1.promises.readdir(dirPath);
        for (const fileName of fileNames) {
            archive.append((0, fs_1.createReadStream)(`${dirPath}/${fileName}`), { name: fileName });
        }
    }
    archive.pipe(stream);
    const isFinished = new Promise(resolve => archive.on('close', resolve));
