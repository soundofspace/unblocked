...
async function importOsVersions() {
    const agent = new unblocked_agent_1.Agent({ options: { disableMitm: true } });
    try {
        const filePath = (0, paths_1.getDataFilePath)('manual/osReleaseDates.json');
        const existing = await (0, fileUtils_1.readFileAsJson)(filePath);
        const page = await agent.newPage();
        await page.goto('https://robservatory.com/a-full-history-of-macos-os-x-release-dates-and-rates/');
        ...
        return results;
            ...
            for (const version of versions) {
                ...
                if (!version.version.match(/^[\d.]+$/))
                    continue;
                existing[key] ?? (existing[key] = moment(`${version.date} ${version.year}`, 'MMM DD YYYY').format('YYYY-MM-DD'));
            }
        }
        {
    ...
    return rows.filter(Boolean);
            ...
            for (const version of versions) {
                if (!version.version[0].includes('Windows'))
                    continue;
                if (Number(version.date.split('-').shift()) < 2009)
                    continue;
                version.version[0] = version.version[0]
                    ...
                    .replace(/\s/g, '-')
                    .replace(/\./g, '-');
                if (version.version[0].split('-').length === 2)
                    version.version[0] += '-0';
                ...
                if (!version.date.match(/^[\d-]+$/))
                    continue;
                existing[key] ?? (existing[key] = version.date);
            }
        }
...
exports.default = importOsVersions;
//# sourceMappingURL=importOsVersions.js.map